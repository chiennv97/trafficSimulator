...
public class OtoTurnLeft extends JPanel {
    ...
    int stopline;
    int countCar = 0;
    public OtoTurnLeft(int positionY, int getX, int sizeCar, java.awt.Color color, int turnX, int size, boolean typeX,  int distance, int stopline){
        ...
        this.rd = new Random();
        this.postions.add(new PositionExtend(this.rd.nextInt(distance) + size, turnX, this.positionY,false));
        for( int i=1; i<numberCar; i++){
            this.postions.add(new PositionExtend(postions.get(i-1).x + this.rd.nextInt(distance)+120, turnX, this.positionY, false));
        }
    }
